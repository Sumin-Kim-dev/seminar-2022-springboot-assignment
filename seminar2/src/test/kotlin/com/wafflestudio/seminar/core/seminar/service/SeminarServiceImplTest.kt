...
internal class SeminarServiceImplTest @Autowired constructor(
    ...
    fun editSeminar() {
        ...
        // when - 세미나의 정보를 수정하면
        val changeSeminar = EditSeminarRequest(seminar.id, name= null, capacity = 20, count = null, time = LocalTime.of(10, 20), online = null)
    ...
    fun getSeminar() {
        ...
        // when - 세미나를 부르면
        val foundseminar = seminarService.getSeminar(instructor.id, seminar.id)
    ...
    fun getAllSeminars() {
        ...
        (1..10).map { 
            ...
            createSeminar(instructor)
        }
        val participant = userTestHelper.createParticipant("participant@naver.com")

        // when - 세미나를 모두 부르면
        val seminars = seminarService.getAllSeminar(participant!!.id, name = "", order = "")

        // then - 모든 세미나가 조회된다
        assertThat(seminars.size).isEqualTo(10)
    }
    ...
    private fun createSeminar(instructor: UserEntity): SeminarEntity {
        val seminar = SeminarEntity("세미나1", 10, 10, LocalTime.of(10, 10), true, instructor.id)
        seminarRepository.save(seminar)
        val userSeminar = UserSeminarEntity(instructor, seminar, User.Role.INSTRUCTOR, LocalDateTime.now(), null, isActive = true)
